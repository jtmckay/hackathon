...
- Blake's judgment layer: policy flex for loyal customers, firm handling for exploitation patterns
...
3. **Dispatch decision engine** — multi-tech evaluation with intent-ranked selection,
   tech confirmation gate, and ops channel reasoning post

4. **Cascading schedule manager** — displaced job handler, tech-confirmation-gated
   customer notifications, customer-tier-aware messaging, schedule rebuild, and Blake
   briefing

5. **Schedule flex system** — daily buffer slot design, over-schedule flagging, and
   post-emergency buffer consumption reporting

6. **Blake's judgment layer** — policy flex logic for loyal customers, exploitation
   pattern detection, escalation flags to ops channel

7. **System prompt with Blake's intent statements** — the core artifact encoding his
   business judgment, relational wisdom, and customer satisfaction philosophy

8. **Sample data set** — four tech profiles, eight Monday jobs (with deliberate flex
   built in), customer tiers, complaint history indicators, and job catalog

9. **Stress-tested curveball coverage** — twelve-plus edge cases validated and
   prompt-tuned, including loyalty vs. exploitation scenarios

10. **Demo-ready state** — clean Monday schedule with visible flex, rehearsed flow,
    two-screen setup
